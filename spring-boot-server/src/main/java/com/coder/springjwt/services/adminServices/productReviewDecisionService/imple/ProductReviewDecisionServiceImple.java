package com.coder.springjwt.services.adminServices.productReviewDecisionService.imple;
        ...
        try {
            ...
            if(reviewDecision && sellerProduct != null) {
                userRepository.findByUsername(sellerProduct.getSellerUserName()).orElseThrow(() ->
                        new UsernameNotFoundException(AdminMessageResponse.USERNAME_NOT_FOUND));
                ...
                return ResponseGenerator.generateSuccessResponse(hashMap,AdminMessageResponse.SUCCESS);
            }
            else{
                throw new RuntimeException(AdminMessageResponse.SOMETHING_WENT_WRONG);
            }
        }
        catch (Exception e)
        {
            e.printStackTrace();
            return ResponseGenerator.generateBadRequestResponse(AdminMessageResponse.SOMETHING_WENT_WRONG);
        }
    }
